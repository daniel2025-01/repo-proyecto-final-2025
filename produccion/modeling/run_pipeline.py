...
"""Entrenamiento de modelos de predicción de ventas."""
...
from .model_configs import MODEL_CONFIGS  # asumiendo que definiste esto


def train_models_with_mlflow(features, labels) -> Dict[str, Any]:
    """Entrena varios modelos, registra todo en MLflow y devuelve info del campeón.

    Args:
        features: Matriz X de entrenamiento.
        labels: Vector y de entrenamiento.

    Returns:
        Diccionario con info del modelo campeón.
    """
    mlflow.set_experiment("Proyecto Final")
    ...
    with mlflow.start_run(run_name="train_pipeline_produccion") as parent_run:
        for model_name, cfg in MODEL_CONFIGS.items():
            ...
            for config_id, param_dict in enumerate(cfg["params"], start=1):
                with mlflow.start_run(
                    ...
                    full_pipeline = Pipeline(steps=[
                        ("features", config.FEATURE_PIPELINE),  # o feature_pipeline
                        ("model", model),
                    ])

                    full_pipeline.fit(features, labels)
                    y_pred = full_pipeline.predict(features)
        ...
        mlflow.log_metric("best_rmse_train", best_rmse)
